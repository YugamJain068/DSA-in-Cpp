...
bool DFS(int node,unordered_map<int,bool>&vis,unordered_map<int,list<int>>&adj,unordered_map<int,bool>&DFSvis){
    vis[node]=1;
    DFSvis[node]=1;
    for(auto neighbour:adj[node]){
        if(!vis[neighbour]){
            bool cycledetected=DFS(neighbour,vis,adj,DFSvis);
            if(cycledetected){
                return true;
            }
        }
